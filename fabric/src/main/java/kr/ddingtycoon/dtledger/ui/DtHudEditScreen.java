...
/**
 * HUD 위치 편집 화면. 실제 HUD 를 미리보기로 그리고 드래그로 이동.
 * 방향키 1px(Shift 10px) 미세조정, 닫으면 config 저장.
 */
public final class DtHudEditScreen extends Screen {
    ...
    public DtHudEditScreen(DtConfig config, LedgerHud hud) {
        this(config, hud, null);
    }

    public DtHudEditScreen(DtConfig config, LedgerHud hud, Screen parent) {
        super(Text.literal("빌띵 HUD 위치 편집"));
        this.config = config;
        this.hud = hud;
        this.parent = parent;
    }
    ...
    protected void init() {
        pullOnScreen();
        addDrawableChild(ButtonWidget.builder(Text.literal("기본값"), b -> {
            config.hudScale = 1.0f;
            moveToDefaultCorner();
        }).dimensions(this.width / 2 - 154, this.height - 30, 70, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.literal("작게 −"), b -> addScale(-DtConfig.HUD_SCALE_STEP))
                .dimensions(this.width / 2 - 78, this.height - 30, 70, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.literal("크게 ＋"), b -> addScale(DtConfig.HUD_SCALE_STEP))
                .dimensions(this.width / 2 - 2, this.height - 30, 70, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.literal("완료"), b -> this.close())
                .dimensions(this.width / 2 + 74, this.height - 30, 70, 20).build());
    }
    ...
    private void pullOnScreen() {
        int[] wh = hud.measureScaled(this.textRenderer);
        lastW = wh[0];
        lastH = wh[1];
    ...
    /** 배율 증감(범위 고정). 소수 오차가 누적되지 않게 스텝 단위로 반올림. */
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawText(this.textRenderer, hint,
                (this.width - this.textRenderer.getWidth(hint)) / 2, 16, MUTED, false);
        String pos = "X " + config.hudX + "   Y " + config.hudY
                + "   크기 " + Math.round(config.hudScaleClamped() * 100) + "%";
        ctx.drawText(this.textRenderer, pos,
                (this.width - this.textRenderer.getWidth(pos)) / 2, 30, GOLD, false);

        // 실제 HUD 미리보기 + 하이라이트 테두리
        int[] wh = hud.drawScaledHud(ctx, this.textRenderer, config.hudX, config.hudY);
        lastW = wh[0];
        lastH = wh[1];
        ...
        for (int i = 1; i <= 3; i++) { // 사선 그립 무늬
            int off = i * 2;
            ctx.fill(hx + HANDLE - off, hy + HANDLE - 1, hx + HANDLE, hy + HANDLE - 1 + 1, GOLD);
            ctx.fill(hx + HANDLE - 1, hy + HANDLE - off, hx + HANDLE, hy + HANDLE - off + 1, GOLD);
        }
    }
    ...
    /**
     * 커서가 HUD 좌상단에서 얼마나 떨어졌는지로 배율을 정한다(비율 유지).
     * 가로/세로 중 더 많이 끈 쪽을 따라가 모서리가 커서를 자연스럽게 쫓아옴.
     * 휠·버튼과 달리 스텝에 안 묶여 자유롭게 조절된다.
     */
    private void resizeTo(double mx, double my) {
        int[] base = hud.measure(this.textRenderer); // 배율 1 기준 크기
    ...
    public void close() {
        config.save();
        if (this.client != null) {
            this.client.setScreen(parent); // 정산 창(또는 게임)으로 복귀
        } else {
            super.close();
        }
    }

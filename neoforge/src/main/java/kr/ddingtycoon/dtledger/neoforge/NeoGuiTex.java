...
public final class NeoGuiTex {
    ...
    public static final int RULE     = 0x574A3420;
    public static final int TRACK    = 0x294A3420;

    public static ResourceLocation id(String name) {
        return ResourceLocation.fromNamespaceAndPath("dtledger", name);
    }

    public static void sprite(GuiGraphics ctx, String name, int x, int y, int w, int h) {
        ctx.blitSprite(RenderType::guiTextured, id(name), x, y, w, h);
    }

    /** 가로 방향 원본 크기 타일 반복(9-slice 아닌 반복 텍스처, 예: 구분선). 우측을 scissor 로 자름. */
    public static void tileH(GuiGraphics ctx, String name, int left, int right, int y, int tileW, int tileH) {
    ...
    public static void tileV(GuiGraphics ctx, String name, int x, int top, int bottom, int tileW, int tileH) {
    ...
    public static String icon(String category) {
        ...
        return switch (category) {
            ...
            case "영혼 명품"   -> "icon_yeonghon";    // 오렌지 명품 핸드백(버킨 모티브, 로고 없음)
